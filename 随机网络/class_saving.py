...
class Vrp():
    def __init__(self):
        ...
        self.Routes = []  # 路线
        self.Cost = 0  # 总路程
        self.tons = 500  # 车辆载重(校园里的话，设成18就可以了,其实也可以设的很大，假设很多车同时送就行)
        self.distanceLimit=1000000 #这里先不考虑距离约束，假设只有容量影响

    def datainput(self, distance_matrix_file="result/distance_matrix.csv", network_file="result/network.csv"):
        # 读取距离矩阵
    ...
    def plotRoutes(self, network_file="result/network.csv"):
        network_data = pd.read_csv(network_file)
        ...
        plt.scatter(coords[0][0], coords[0][1], c='red', label='Distribution Center',zorder=5)
